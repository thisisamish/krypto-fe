    ...
    <div class="p-4 sm:p-6 bg-white rounded-lg shadow">
      ...
      <ng-container *ngIf="!loading && !error">
        ...
        <div class="flex justify-between items-center mt-4" *ngIf="totalPages > 1">
           <button (click)="previousPage()" [disabled]="currentPage === 0" class="btn btn-ghost">Previous</button>
           <span>Page {{ currentPage + 1 }} of {{ totalPages }}</span>
           <button (click)="nextPage()" [disabled]="currentPage >= totalPages - 1" class="btn btn-ghost">Next</button>
        </div>
      </ng-container>
    </div>
  `,
